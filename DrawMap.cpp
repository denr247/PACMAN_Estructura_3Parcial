...
void draw_map(const std::array<std::array<Cell, MAP_HEIGHT>, MAP_WIDTH>& i_map, sf::RenderWindow& i_window)
{
    ...
    textureFruit.loadFromFile("Resources/Images/apple16.png");
                    spriteFruit.setTexture(textureFruit);
	...
	for (unsigned char a = 0; a < MAP_WIDTH; a++)
	{
		for (unsigned char b = 0; b < MAP_HEIGHT; b++)
		{
			...
			switch (i_map[a][b])
			{
				...
				case Cell::Fruit:
				{
                    spriteFruit.setPosition(static_cast<float>(CELL_SIZE * a), static_cast<float>(CELL_SIZE * b)); // Posiciona la fruta correctamente
                    spriteFruit.setScale(0.5f, 0.5f);

                    i_window.draw(spriteFruit);
